...
- `core`: Ubuntu, Node, Git, and a pinned Claude Code ARM64 fallback. PocketForge
...
- `android`: a portable JDK 17, Android SDK, ARM64 build tools, Gradle, the
  offline Maven repository, and PocketForge's global ARM64 AAPT2 configuration.
- `cpp`: GCC, G++, make, CMake, and GDB.
- `php`: PHP CLI, common extensions, and Composer.
- Claude Code remains separate so PocketForge can install or update Anthropic's
...
32-bit-only ARM devices. PocketForge must continue checking for `arm64-v8a` before
installation.
