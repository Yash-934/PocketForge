...
class RuntimeExecutionService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action ?: ACTION_START) {
            ...
            ACTION_COMPLETE -> finishTask(
                ...
                detail = intent?.getStringExtra(EXTRA_DETAIL) ?: "PocketForge finished working in $projectName.",
                failed = false,
            )
            ACTION_FAILED -> finishTask(
                title = "Task needs attention",
                detail = intent?.getStringExtra(EXTRA_DETAIL) ?: "PocketForge could not finish the task.",
                failed = true,
            )
            ACTION_CANCELLED -> {
                taskRunning = false
    ...
    private fun acquireWakeLock() {
        ...
        wakeLock = getSystemService(PowerManager::class.java)
            .newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "com.pocketforge.mobile:active-coding-task")
    ...
    companion object {
        const val ACTION_START = "com.pocketforge.mobile.START_RUNTIME"
        const val ACTION_STOP = "com.pocketforge.mobile.STOP_RUNTIME"
        const val ACTION_PROGRESS = "com.pocketforge.mobile.PROGRESS_RUNTIME"
        const val ACTION_COMPLETE = "com.pocketforge.mobile.COMPLETE_RUNTIME"
        const val ACTION_FAILED = "com.pocketforge.mobile.FAIL_RUNTIME"
        const val ACTION_CANCELLED = "com.pocketforge.mobile.CANCEL_RUNTIME"
