...
object RuntimeLaunchConfigBuilder {
    fun build(profile: ProviderProfile, authToken: String? = null, localGatewayUrl: String? = null): RuntimeLaunchConfig {
        ...
        when (profile.kind.protocol) {
            ...
            com.pocketforge.mobile.model.ProviderProtocol.ANTHROPIC_GATEWAY -> {
                environment["ANTHROPIC_BASE_URL"] = profile.baseUrl.trimEnd('/')
                environment["ANTHROPIC_MODEL"] = profile.model
            }
            com.pocketforge.mobile.model.ProviderProtocol.OPENROUTER -> {
                environment["ANTHROPIC_BASE_URL"] = profile.baseUrl.trimEnd('/')
                environment["ANTHROPIC_MODEL"] = profile.model
            }
        ...
        if (profile.kind.protocol != com.pocketforge.mobile.model.ProviderProtocol.CLAUDE_LOGIN) {
            ...
            if (!authToken.isNullOrBlank()) {
                ...
                if (profile.kind == com.pocketforge.mobile.model.ProviderKind.LLM_ROUTER) {
